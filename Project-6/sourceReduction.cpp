...
if( status != CL_SUCCESS )
  ...
  status = clSetKernelArg( kernel, 3, sizeof(float), NULL ); //local prods array
	if( status != CL_SUCCESS )
	...
	if( status != CL_SUCCESS )
			fprintf( stderr, "clEnqueueNDRangeKernel failed: %d .... NUM_ELEMENTS: %8d LOCAL_SIZE: %4d \n", status, NUM_ELEMENTS, LOCAL_SIZE );
	Wait( cmdQueue );
	double time1 = omp_get_wtime( );

	// 12. read the results buffer back from the device to the host:
	status = clEnqueueReadBuffer( cmdQueue, dD, CL_TRUE, 0, dSize, hD, 0, NULL, NULL );
	if( status != CL_SUCCESS )
  ...
  fprintf(stderr, "sum: %lf\n", sum);
	...
	printf("%d,%d,%.3lf\n", LOCAL_SIZE, NUM_ELEMENTS,
				 (double)NUM_ELEMENTS/(time1-time0)/1000000000.0 );
